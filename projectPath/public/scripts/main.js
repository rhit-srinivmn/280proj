...
rhit.fbStudentLogManager = null;
rhit.fbSingleLogManager = null;
...
rhit.ListPageController = class {
	constructor() {
		...
		document.querySelector("#submitAddLog").addEventListener("click", (event) => {
			...
			const log = document.querySelector("#inputLog").value;
			...
			rhit.fbStudentLogManager.add(subject, log, name, date, comment, grade);
		});
		...
		// Start listening!
		rhit.fbStudentLogManager.beginListening(this.updateList.bind(this));
	...
	updateList() {
		...
		console.log(`Num Posts = ${rhit.fbStudentLogManager.length}`);
		const newList = htmlToElement('<div id="studentLogContainer"></div>');
		for (let i = 0; i < rhit.fbStudentLogManager.length; i++) {
			const lg = rhit.fbStudentLogManager.getPostAtIndex(i);
	...
	_createCard(log) {
		...
		<div class="card-body" style = "border-radius: 10%">
			...
			<h5 class="card-subtitle mb-2">${log.post}</h5>
...
rhit.Post = class {
	constructor(id, subject, post, name, date, comment, grade) {
		this.id = id;
...
rhit.FbStudentLogManager = class {
	constructor(uid) {
		this._uid = uid;
	...
	getPostAtIndex(index) {
		...
		const lg = new rhit.Post(docSnapshot.id,
...
rhit.DetailPageController = class {
	constructor() {
		...
		document.querySelector("#submitEditPost").addEventListener("click", (event) => {
			...
			const comment = document.querySelector("#inputComment").value;
			const grade = document.querySelector("#inputGrade").value;
			rhit.fbSingleLogManager.update(subject, post, name, date, comment, grade);
		});

		$("#editPostDialog").on("show.bs.modal", (event) => {
			// Pre animation
			document.querySelector("#inputSubject").value = rhit.fbSingleLogManager.subject;
			document.querySelector("#inputPost").value = rhit.fbSingleLogManager.post;
			document.querySelector("#inputName").value = rhit.fbSingleLogManager.name;
			document.querySelector("#inputDate").value = rhit.fbSingleLogManager.date;
			document.querySelector("#inputComment").value = rhit.fbSingleLogManager.comment;
			document.querySelector("#inputGrade").value = rhit.fbSingleLogManager.grade;
		});
		...
		document.querySelector("#submitDeletePost").addEventListener("click", event => {
			rhit.fbSingleLogManager.delete().then(() => {
				console.log("Document successfully deleted!");
				window.location.href = "/list.html";
			}).catch(error => console.error("Error removing document: ", error));
		});

		rhit.fbSingleLogManager.beginListening(this.updateView.bind(this));
	}
	updateView() {
		document.querySelector("#cardSubject").innerHTML = rhit.fbSingleLogManager.subject;
		document.querySelector("#cardPost").innerHTML = rhit.fbSingleLogManager.post;
		document.querySelector("#cardName").innerHTML = rhit.fbSingleLogManager.name;
		document.querySelector("#cardDate").innerHTML = rhit.fbSingleLogManager.date;
		document.querySelector("#cardComment").innerHTML = rhit.fbSingleLogManager.comment;
		document.querySelector("#cardGrade").innerHTML = rhit.fbSingleLogManager.grade;

		if(rhit.fbSingleLogManager.user == rhit.fbAuthManager.uid || rhit.fbAuthManager.uid == masterAuth){
			document.querySelector("#cardGrade").style.display = "flex";
			document.querySelector("#menuEdit").style.display = "flex";
			document.querySelector("#menuDelete").style.display = "flex";
			if(rhit.fbSingleLogManager.grade == "10" || rhit.fbSingleLogManager.grade == '9'){
				document.body.style.backgroundColor = "green";
			}else if(rhit.fbSingleLogManager.grade == "7" || rhit.fbSingleLogManager.grade == '8'){
...
rhit.initializePage = function(){
	const urlParams = new URLSearchParams(window.location.search);
		...
		rhit.fbStudentLogManager = new rhit.FbStudentLogManager(uid);
	...
	if (document.querySelector("#detailPage")) {
		...
		rhit.fbSingleLogManager = new rhit.FbSingleLogManager(postId);
